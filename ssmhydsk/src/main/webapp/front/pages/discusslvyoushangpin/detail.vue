...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusslvyoushangpin');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discusslvyoushangpin');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discusslvyoushangpin', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscusslvyoushangpin', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscusslvyoushangpin/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discusslvyoushangpin', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discusslvyoushangpin', this.detail);
				this.$utils.msg('审核成功');
